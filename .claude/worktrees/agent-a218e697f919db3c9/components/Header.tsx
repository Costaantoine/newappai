  ...
  return (
    ...
    >
      <div className="text-2xl font-bold tracking-tighter">
        <Link href="/" className="flex items-center">
          {logoUrl ? (
            ...
            <>
              <span className="text-white" data-section="site-logo-text">{settings?.site?.logo_text || 'NewApp'}</span>
      ...
      <nav className="hidden md:flex space-x-8 text-sm font-medium text-slate-300">
        ...
        <Link href="/produits" className={`${linkClass('/produits')} nav-link`}><span data-section="nav-products">Produits</span></Link>
      ...
      <div className="flex items-center space-x-4">
        ...
        {settings?.header?.show_cart && (
          <button onClick={() => window.dispatchEvent(new Event('open-cart'))} className="text-slate-400 hover:text-[var(--color-primary)] transition" title="Panier">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
