...
export default function ZonesManager() {
  ...
  const handleDeleteZone = async (id: string, e?: React.MouseEvent) => {
    ...
    try {
      const res = await fetch(`/api/supabase/zones?id=${id}`, { method: 'DELETE' })
      if (res.ok) {
        fetchData()
  ...
  const handleSaveCard = async () => {
    ...
    try {
      const res = await fetch(`/api/supabase/cards?id=${editingCard.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteCard = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/supabase/cards?id=${id}`, { method: 'DELETE' })
      if (res.ok) {
        fetchData()
      }
    } catch (error) {
          ...
          <div className="bg-slate-900 rounded-2xl p-6 w-full max-w-2xl border border-white/10 max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {editingZone.title_key && (
                  <p className="text-xs text-slate-500 italic mt-1">
                    Texte actuel : {texts.find(t => t.key === editingZone.title_key)?.fr || '(clé introuvable)'}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  list="text-keys"
                />
                {editingZone.subtitle_key && (
                  <p className="text-xs text-slate-500 italic mt-1">
                    Texte actuel : {texts.find(t => t.key === editingZone.subtitle_key)?.fr || '(clé introuvable)'}
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="text-slate-400 text-sm">Icône / Pictogramme</label>
                <input
                  type="text"
                  value={editingZone.icon_url || ''}
                  onChange={(e) => setEditingZone({...editingZone, icon_url: e.target.value})}
                  className="w-full bg-slate-800 border border-white/10 rounded-lg px-4 py-2 text-white"
                  placeholder="URL de l'icône (laisser vide pour l'icône SVG par défaut)"
                />
                <input
                  type="file"
                  accept="image/*"
                  onChange={async (e) => {
                    ...
                    try {
                      ...
                      const data = await res.json()
                      if (data.url) setEditingZone((prev: any) => ({...prev, icon_url: data.url}))
                    } catch {} finally { e.target.value = '' }
                  }}
                  className="w-full bg-slate-800 border border-white/10 rounded-lg px-4 py-2 text-white mt-2"
                />
                {editingZone.icon_url && (
                  <div className="mt-2">
                    <img src={editingZone.icon_url} alt="" className="w-16 h-16 object-cover rounded-xl" />
                    <button onClick={() => setEditingZone((prev: any) => ({...prev, icon_url: ''}))} className="text-red-400 text-xs mt-1 hover:text-red-300 block">Supprimer</button>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {editingZone.cta_key && (
                  <p className="text-xs text-slate-500 italic mt-1">
                    Texte actuel : {texts.find(t => t.key === editingZone.cta_key)?.fr || '(clé introuvable)'}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  list="text-keys"
                />
                {editingZone.newtab_key && (
                  <p className="text-xs text-slate-500 italic mt-1">
                    Texte actuel : {texts.find(t => t.key === editingZone.newtab_key)?.fr || '(clé introuvable)'}
                  </p>
                )}
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  list="text-keys"
                />
                {editingCard.title_key && (
                  <p className="text-xs text-slate-500 italic mt-1">
                    Texte actuel : {texts.find(t => t.key === editingCard.title_key)?.fr || '(clé introuvable)'}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                />
                {editingCard.description_key && (
                  <p className="text-xs text-slate-500 italic mt-1">
                    Texte actuel : {texts.find(t => t.key === editingCard.description_key)?.fr || '(clé introuvable)'}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  list="text-keys"
                />
                {editingCard.badge_key && (
                  <p className="text-xs text-slate-500 italic mt-1">
                    Texte actuel : {texts.find(t => t.key === editingCard.badge_key)?.fr || '(clé introuvable)'}
                  </p>
                )}
              </div>
