    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-6 right-6 w-96 max-w-[calc(100vw-3rem)] z-50">
          <div className="glass rounded-[2rem] border-violet-500/30 shadow-2xl overflow-hidden flex flex-col" style={{ maxHeight: '70vh' }}>
              ...
              {suggestContact && (
                <div className="flex justify-center">
                  <button
                    ...
                    className="bg-violet-500 text-white px-4 py-2 rounded-full text-sm font-bold hover:opacity-90 transition-all"
                  >
                    {t.fillForm} →
                  </button>
                </div>
            ...
            <form onSubmit={handleSubmit} className="p-4 border-t border-white/10 bg-slate-950/80">
              <div className="flex items-center space-x-2">
                <button
                  ...
                  aria-label={isListening ? 'Arrêter la dictée vocale' : 'Dicter par voix'}
                >
                ...
                <input
                  ...
                  className="flex-1 bg-slate-800 border border-white/10 rounded-full px-4 py-3 text-white placeholder-slate-400 focus:outline-none focus:border-violet-500 transition text-sm"
                  disabled={isLoading}
                />
                <button
                  type="submit"
                  disabled={isLoading || !input.trim()}
                  className="p-3 rounded-full bg-violet-500 text-white hover:bg-violet-400 transition disabled:opacity-50 disabled:cursor-not-allowed"
