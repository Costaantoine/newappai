  ...
  const zoneFields = [
    ...
    { name: 'subtitle_key', label: 'Cle sous-titre', type: 'select', options: texts.map(t => ({ value: t.key, label: t.key })) },
    {
      name: 'color', label: 'Couleur', type: 'select', options: [
        ...
        { value: 'pink', label: 'Rose' }, { value: 'amber', label: 'Ambre' }, { value: 'rose', label: 'Rouge' }
      ]
    },
    { name: 'icon_url', label: 'Icône / Pictogramme', type: 'image-upload' },
    { name: 'url', label: 'URL de destination (bouton Découvrir)', type: 'text', placeholder: '/solutions' },
  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-20">
      ...
      <header className="fixed w-full z-50 glass py-4 px-8 flex justify-between items-center border-b border-white/10 bg-slate-900">
        <div className="flex items-center gap-4">
          <span className="text-2xl font-bold tracking-tighter">
            <span className="text-white">{settings?.site?.logo_text || 'NewApp'}</span><span className="text-violet-400">AI</span>
          </span>
          <span className="text-slate-400">Admin</span>
        </div>
        <nav className="hidden md:flex gap-4">
          <Link href="/admin/accueil" className="text-violet-400 font-medium">Accueil</Link>
      ...
      <div className="pt-24 px-6 max-w-7xl mx-auto">
        ...
        <div className="bg-violet-500/10 border border-violet-500/20 rounded-xl p-4 mb-8 flex items-center justify-between">
          <p className="text-violet-300 text-sm">Conseil : survolez les champs avec la souris pour voir une explication détaillée.</p>
          <Link href="/" className="text-violet-400 hover:text-violet-300 text-sm font-medium underline">Voir le site →</Link>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                {settings?.site?.logo_image_url ? (
                  ...
                  <div className="w-16 h-16 rounded-xl flex items-center justify-center text-white font-bold text-2xl" style={{ backgroundColor: settings?.site?.primary_color || '#8b5cf6' }}>
            ...
            <div className="mt-6 pt-6 border-t border-white/10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-800/50 p-3 rounded-lg">
                  ...
                  ]) }} className="mt-2 px-3 py-1 bg-violet-500 text-white rounded text-xs hover:bg-violet-400">Modifier</button>
                </div>
                <div className="bg-slate-800/50 p-3 rounded-lg">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="flex gap-2">
                  <button className="px-4 py-2 rounded text-white text-sm" style={{ backgroundColor: settings?.buttons?.primary_color || '#8b5cf6', borderRadius: settings?.buttons?.border_radius || 8 }}>Primaire</button>
                  <button className="px-4 py-2 rounded text-white text-sm" style={{ backgroundColor: settings?.buttons?.secondary_color || '#a855f7', borderRadius: settings?.buttons?.border_radius || 8 }}>Secondaire</button>
                </div>
                <div>
        ...
        <section className="mb-8">
          ...
          <div className="glass p-6 rounded-2xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-lg bg-slate-700 flex items-center justify-center">
                  <svg className={`w-6 h-6 text-violet-400 ${settings?.animations?.enabled ? 'animate-pulse' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /></svg>
                </div>
                <div>
            ...
            <div className="mt-4 p-4 bg-slate-800/50 rounded-xl">
              <div className="flex flex-wrap items-center gap-4 mb-4">
                <input
                  ...
                  className="text-sm text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-violet-600 file:text-white hover:file:bg-violet-500"
                />
        ...
        <section className="mb-8">
          ...
          <div className="flex justify-end mb-4">
            <AdminButton showAdd addLabel="Zone" onAdd={() => openAdd('zone', 'Ajouter une Zone', zoneFields, { color: 'violet', order: zones.length + 1, active: true })} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {zones.filter(z => z.active).sort((a, b) => a.order - b.order).map((zone) => (
              <div key={zone.id} className="glass p-5 rounded-2xl relative">
                ...
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold ${zone.color === 'purple' ? 'bg-purple-500' : 'bg-violet-500'} text-white`}>
                    {zone.badge}
                  </div>
                  <div>
                ...
                <div className="mt-4 pt-4 border-t border-white/10">
                  <button onClick={() => openAdd('card', 'Ajouter un Module', cardFields, { zone_id: zone.id, order: cards.filter(c => c.zone_id === zone.id).length + 1, active: true })} className="text-violet-400 text-sm hover:text-violet-300">
                    + Ajouter un module
                  </button>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {cards.filter(c => c.zone_id === zone.id).sort((a, b) => a.order - b.order).map(card => (
                      <div key={card.id} className="bg-white/5 px-3 py-1 rounded-lg text-xs text-slate-300 flex items-center gap-2">
                        ...
                        <button onClick={() => openEdit('card', card, 'Modifier le Module', cardFields)} className="text-violet-400 hover:text-violet-300">✏️</button>
