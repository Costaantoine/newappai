...
interface AIChatResponse {
  message: string
  provider: 'xiaomi' | 'deepseek' | 'gemini'
}

export async function chatWithAI(
  messages: AIChatMessage[],
  provider: 'xiaomi' | 'deepseek' | 'gemini' = 'xiaomi'
): Promise<AIChatResponse> {
  if (provider === 'xiaomi') {
    return chatWithXiaomi(messages)
  }
...
async function chatWithXiaomi(messages: AIChatMessage[]): Promise<AIChatResponse> {
  const apiKey = process.env.XIAOMI_API_KEY
  if (!apiKey) {
    return { message: 'Clé API Xiaomi non configurée', provider: 'xiaomi' }
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Xiaomi API error: ${response.status}`)
    }
    ...
    console.error('Erreur Xiaomi:', error)
    return { message: 'Erreur de communication avec Xiaomi', provider: 'xiaomi' }
  }
}
