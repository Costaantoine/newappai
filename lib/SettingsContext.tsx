...
interface SiteSettings {
  ...
  sound_hover_url: string
  sound_click_url: string
  hero_texts?: { title?: { fr?: string; en?: string; pt?: string; es?: string }; subtitle?: { fr?: string; en?: string; pt?: string; es?: string }; subtitle2?: { fr?: string; en?: string; pt?: string; es?: string }; cta1?: { fr?: string; en?: string; pt?: string; es?: string }; cta2?: { fr?: string; en?: string; pt?: string; es?: string } }
  contact_page?: { title?: { fr?: string; en?: string; pt?: string; es?: string }; subtitle?: { fr?: string; en?: string; pt?: string; es?: string } }
}
...
export function SettingsProvider({ children, initialSettings }: { children: ReactNode, initialSettings?: any }) {
  const [settings, setSettings] = useState<SiteSettings>(initialSettings ? { ...defaultSettings, ...initialSettings } as SiteSettings : defaultSettings as SiteSettings)
  ...
  // Apply server-side settings on first render (SSR won't run applyStyles,
  // but the inline <style> below will have the correct values)
  const mergedInit = initialSettings
    ? { ...defaultSettings, ...initialSettings }
    : defaultSettings

  const fetchSettings = async () => {
    try {
      // Essayer Supabase d'abord, fallback sur local
