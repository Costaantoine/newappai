...
export default function AdminDashboard() {
  ...
  const deleteProduct = async (id: string) => {
    ...
    await fetch(`/api/supabase/products?id=${id}`, { method: 'DELETE' })
    fetchProducts()
  }
    ...
    <div className="min-h-screen">
      <header className="glass py-4 px-8 flex justify-between items-center sticky top-0 z-50">
        ...
        <div className="flex items-center space-x-4">
          <Link href="/admin/products/new" className="bg-violet-500 text-white px-6 py-2 rounded-full font-bold hover:bg-violet-400 transition">
            + Nouveau produit
          </Link>
          <Link href="/admin/texts" className="text-slate-300 hover:text-violet-400 transition border border-white/20 px-4 py-2 rounded-full">
            Textes
          </Link>
          <Link href="/admin/zones" className="text-slate-300 hover:text-violet-400 transition border border-white/20 px-4 py-2 rounded-full">
            Zones
          </Link>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        {loading ? (
          ...
          <div className="glass p-10 rounded-[2rem] text-center">
            ...
            <Link href="/admin/products/new" className="text-violet-400 hover:text-violet-300 font-bold">
