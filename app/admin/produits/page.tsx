...
export default function AdminProduitsPage() {
  ...
  const handleSave = async (savedData?: any) => {
    ...
    try {
      ...
      if (modalType === 'settings') {
        ...
        endpoint = body.id ? `/api/supabase/products?id=${body.id}` : '/api/supabase/products'
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      let endpoint = `/api/supabase/products?id=${deleteItem.id}`
  ...
  const saveImageSettings = async (newSettings: any) => {
    ...
    await fetch(`/api/supabase/products?id=${updatedProduct.id}`, {
      method: 'PUT',
    ...
    <div className="min-h-screen bg-slate-950 pb-20">
      <header className="fixed w-full z-50 glass py-4 px-8 flex justify-between items-center border-b border-white/10 bg-slate-900">
        ...
        <nav className="hidden md:flex gap-4">
          ...
          <Link href="/admin/produits" className="text-violet-400 font-medium">Produits</Link>
              ...
              return (
                <div key={product.id} className="glass p-6 rounded-2xl relative">
                  <div className="absolute top-4 right-4 flex gap-2 z-10">
                    <AdminButton showEdit onEdit={() => openEdit('product', product, 'Modifier le Produit', productFields)} showDelete onDelete={() => { setDeleteItem({ type: 'product', id: product.id, name: product.title?.fr || product.title?.en || 'Produit' }); setDeleteOpen(true); }} />
                  </div>
                  ...
                  {product.images && product.images[0] && (
                    <div className="relative mb-4 group">
                      ...
                      <button
                        ...
                        className="absolute bottom-2 right-2 p-1.5 bg-slate-800/80 rounded-lg text-white hover:bg-violet-500 opacity-0 group-hover:opacity-100 transition-opacity"
                        title="Réglage image"
                      >
                        🎨
                      </button>
