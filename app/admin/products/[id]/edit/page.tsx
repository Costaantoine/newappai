...
export default function EditProductPage() {
  ...
  const fetchProduct = async (id: string) => {
    const res = await fetch(`/api/supabase/products?id=${id}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/supabase/products?id=${productId}`, {
        method: 'PUT',
        ...
        <form onSubmit={handleSubmit} className="glass p-8 rounded-[2rem] space-y-6">
          ...
          <div>
            ...
            <button
              ...
              disabled={uploading}
              className="w-full border-2 border-dashed border-white/20 rounded-xl py-8 text-slate-400 hover:border-violet-500 hover:text-violet-400 transition"
            >
          ...
          <button
            type="submit"
            disabled={saving}
            className="w-full bg-violet-500 text-white py-3 rounded-full font-bold hover:bg-violet-400 transition disabled:opacity-50"
          >
