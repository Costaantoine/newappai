...
export default function AdminSettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const existingRes = await fetch('/api/supabase/settings')
    ...
    <div className="min-h-screen">
      <header className="glass py-4 px-8 flex justify-between items-center sticky top-0 z-50">
        ...
        <div className="flex items-center space-x-4">
          <Link href="/" target="_blank" className="text-slate-300 hover:text-violet-400 transition">
            Voir le site
          </Link>
          <Link href="/admin/dashboard" className="text-slate-300 hover:text-violet-400 transition">
            Produits
          </Link>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="bg-violet-500 text-white px-6 py-3 rounded-full font-bold hover:bg-violet-400 transition disabled:opacity-50"
          >
        ...
        {activeTab === 'hero' && (
          <>
            ...
            <div className="glass p-8 rounded-[2rem] space-y-8 mb-8">
              <div>
                <h2 className="text-xl font-bold mb-6 text-violet-400">Image Hero</h2>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      ...
                      <input type="range" min="0" max="100" value={settings.hero_opacity} onChange={(e) => setSettings({ ...settings, hero_opacity: Number(e.target.value) })} className="w-full accent-violet-500" />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-slate-300 mb-2">Luminosité: {settings.hero_brightness}%</label>
                      <input type="range" min="50" max="200" value={settings.hero_brightness} onChange={(e) => setSettings({ ...settings, hero_brightness: Number(e.target.value) })} className="w-full accent-violet-500" />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-slate-300 mb-2">Overlay: {settings.hero_overlay_opacity}%</label>
                      <input type="range" min="0" max="100" value={settings.hero_overlay_opacity} onChange={(e) => setSettings({ ...settings, hero_overlay_opacity: Number(e.target.value) })} className="w-full accent-violet-500" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h2 className="text-xl font-bold mb-6 text-violet-400">Textes Hero</h2>
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-semibold text-slate-300 mb-2">Titre principal</label>
                    <input type="text" value={settings.hero_title} onChange={(e) => setSettings({ ...settings, hero_title: e.target.value })} className="w-full bg-slate-950/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-violet-500" placeholder="Laisser vide pour traductions" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-300 mb-2">Sous-titre 1</label>
                    <textarea value={settings.hero_subtitle1} onChange={(e) => setSettings({ ...settings, hero_subtitle1: e.target.value })} rows={2} className="w-full bg-slate-950/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-violet-500" placeholder="Laisser vide pour traductions" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-300 mb-2">Sous-titre 2</label>
                    <textarea value={settings.hero_subtitle2} onChange={(e) => setSettings({ ...settings, hero_subtitle2: e.target.value })} rows={2} className="w-full bg-slate-950/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-violet-500" placeholder="Laisser vide pour traductions" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="glass p-8 rounded-[2rem]">
              <h2 className="text-xl font-bold mb-4 text-violet-400">Presets d'images</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  <button key={preset.name} onClick={() => setSettings({ ...settings, hero_image_url: preset.url })} className={`p-2 rounded-lg border transition ${settings.hero_image_url === preset.url ? 'border-violet-500 bg-violet-500/20' : 'border-white/10 hover:border-white/30'}`}>
        ...
        {activeTab === 'sounds' && (
          <>
            ...
            <div className="glass p-8 rounded-[2rem] mb-8">
              <h2 className="text-2xl font-bold mb-6 text-violet-400">🎵 Musique par Page</h2>
            ...
            {/* Sound Effects */}
            <div className="glass p-8 rounded-[2rem]">
              <h2 className="text-2xl font-bold mb-6 text-violet-400">🔊 Effets Sonores</h2>
