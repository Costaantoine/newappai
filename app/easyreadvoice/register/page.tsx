...
function RegisterForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/easyreadvoice/register', {
        method: 'POST',
      ...
      // Compte créé et confirmé immédiatement : connexion automatique
      const { error: signInError } = await supabase.auth.signInWithPassword({ email, password })
      if (signInError) {
        setError('Compte créé. Essayez de vous connecter.')
        return
      }

      const next = searchParams.get('next')
      router.push(next || '/easyreadvoice/dashboard')
    } catch {
